...
export async function getMediaDetails(type, id) {
  const data = await tmdbFetch(`/${type}/${id}?append_to_response=credits,keywords`);
  ...
  const directors = type === "movie"
    ...
    : (data.created_by || []).map((person) => ({ id: person.id, name: person.name }));
  ...
  return {
    ...
    rating: normalizeNumber(data.vote_average),
    popularity: normalizeNumber(data.popularity),
    language: data.original_language || null,
    runtime: type === "movie" ? normalizeNumber(data.runtime) : normalizeNumber(data.episode_run_time?.[0]),
    ...
    creators: type === "tv" ? data.created_by?.map((person) => person.name) || [] : [],
    studios: (data.production_companies || []).map((company) => ({ id: company.id, name: company.name })),
