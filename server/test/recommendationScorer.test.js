...
function genre(id = "genre-1") {
  return { type: "genre", value: id };
}

test("S-rated history creates positive actor evidence", () => {
  const result = scoreCandidate(
    candidate({ connections: [actor()] }),
    [historyItem({ rating: "S", connections: [actor()] })],
  );
...
test("D-rated history creates negative actor evidence", () => {
  const result = scoreCandidate(
    candidate({ connections: [actor()] }),
    [historyItem({ rating: "D", connections: [actor()] })],
  );
  assert.ok(result.recommendationScore < 0);
  assert.ok(result.strongScore < 0);
});
...
test("C-rated history is neutral and does not create evidence", () => {
  const result = scoreCandidate(
    candidate({ connections: [actor()] }),
    [historyItem({ rating: "C", connections: [actor()] })],
  );
...
test("repeated actor evidence is capped rather than stacked", () => {
  ...
  assert.ok(repeated.recommendationScore <= single.recommendationScore + 0.001);
  assert.ok(repeated.recommendationScore > 0);
  assert.ok(repeated.recommendationScore <= RECOMMENDATION_SCORING_POLICY.channelCaps.actor + RECOMMENDATION_SCORING_POLICY.maxQualityBonus);
});

test("multiple actor matches cannot overwhelm independent genre evidence", () => {
  const actorHistory = Array.from({ length: 20 }, (_, index) =>
    historyItem({ id: index + 1, rating: "S", connections: [actor(`a-${index % 4}`)] }),
  );
  const actorCandidate = candidate({
    id: 1,
    connections: [actor("a-0"), actor("a-1"), actor("a-2"), actor("a-3")],
  });
  const genreCandidate = candidate({
    id: 2,
    connections: [genre("drama"), genre("thriller")],
  });
  const history = [
    ...actorHistory,
    historyItem({ id: 50, rating: "S", connections: [genre("drama"), genre("thriller")] }),
  ];
...
test("scores remain finite under repeated evidence", () => {
  const history = Array.from({ length: 100 }, (_, index) =>
    historyItem({ id: index + 1, rating: index % 2 === 0 ? "S" : "D", connections: [actor("a")] }),
  );
